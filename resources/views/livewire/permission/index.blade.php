    ...
    <div class="overflow-x-auto ">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-indigo-900/50">
            <thead class="bg-gradient-to-r from-indigo-600 to-purple-600 dark:from-gray-800 dark:to-gray-900">
                <tr>
                    <th class="px-6 py-3 text-left text-xs font-medium text-white dark:text-indigo-200 uppercase tracking-wider" scope="col">Permissions</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-white dark:text-indigo-200 uppercase tracking-wider" scope="col">Created At</th>
                    <th class="px-6 py-3 text-right text-xs font-medium text-white dark:text-indigo-200 uppercase tracking-wider" scope="col">Actions</th>
>>>>>>> 022505578b6891fe056041fbc13e28ad4c1cc169
                </tr>
            </thead>
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-indigo-900/30">
                @foreach ($permissions as $key => $permission)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/80 transition-colors duration-150">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex flex-wrap gap-2">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800 dark:bg-indigo-900/50 dark:text-indigo-200 shadow-sm">
                                    {{ $permission->name ?? '' }}
                                </span>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2 ">
                            <x-customecomponent.actionbutton 
                                id="{{ $permission->id }}" 
                                variant="primary" 
                                color="amber-500 hover:amber-600" 
                                button="Edit" 
                                action="edit"
                            ...
                            <x-customecomponent.actionbutton 
                                id="{{ $permission->id }}" 
                                variant="primary" 
                                color="emerald-500 hover:emerald-600" 
                                button="Show" 
                                action="show" 
                            />
